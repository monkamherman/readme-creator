...
export default function CurrentLocation() {
  ...
  const checkLocationPermission = async () => {
    ...
    if (status === 'granted') {
      ...
      // Permission refusée mais peut redemander
      // Afficher l'écran Autoriser la localisation
  ...
  async function getCurrentLocationOnStart() {
    ...
    // Permission accordée, continuer avec la logique de localisation
  ...
  async function onRequestPermissionHandler() {
    ...
    if (!permission_response?.canAskAgain) {
      // Optionnellement inviter l'utilisateur à ouvrir les paramètres
      if (Platform.OS === 'ios') {
        Linking.openURL('app-settings:') // Lien profond iOS vers les paramètres de l'application
      } else {
  ...
  // Ajouter un petit délai pour éviter le clignotement de l'écran de permission
  useEffect(() => {
    const timer = setTimeout(() => {
      setIsInitialLoading(false)
    }, 1000) // Attendre 1 seconde avant d'afficher l'écran de permission
